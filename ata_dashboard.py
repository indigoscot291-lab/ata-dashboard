...
SHEET_URLS = {
    "1st Degree Black Belt 50-59": "https://docs.google.com/spreadsheets/d/1tCWIc-Zeog8GFH6fZJJR-85GHbC1Kjhx50UvGluZqdg/export?format=csv",
    "2/3 Degree Black Belt 40-49": "https://docs.google.com/spreadsheets/d/1W7q6YjLYMqY9bdv5G77KdK2zxUKET3NZMQb9Inu2F8w/export?format=csv"
}

GROUPS = list(SHEET_URLS.keys())
...
def gather_data(group, selected):
    combined = {ev: [] for ev in EVENT_NAMES}
    code = "W01D" if group == "1st Degree Black Belt 50-59" else "W23C"
    # World
...
sheet_df = fetch_sheet(SHEET_URLS[group])

selected = st.selectbox("Select region:", REGIONS)

name_search = st.text_input("Search competitor name (optional):").strip().lower()

go = st.button("Go")

if go:
    with st.spinner("Loading standings..."):
        raw, has_results = gather_data(group, selected)
    ...
    if not has_results:
        ...
        for ev in EVENT_NAMES:
            rows = data.get(ev, [])
            if name_search:
                rows = [r for r in rows if name_search in r["Name"].lower()]
            if rows:
                st.subheader(ev)
                # Table header
                ...
                for row in rows:
                    ...
                    # Dropdown with tournament breakdown
                    comp_data = sheet_df[
                    ...
                    if not comp_data.empty:
                        ...
                        if is_mobile == "No":
                            ...
                            st.write(f"Points breakdown for {row['Name']}:")
                            st.dataframe(comp_data, use_container_width=True, hide_index=True)
