...
def gather_data(group_key: str, selected_region: str, selected_regions_list=None):
    ...
    if selected_regions_list:
        any_data = False
        for region_name in selected_regions_list:
...
district_choice = st.selectbox("Select District (optional):", [""] + sorted(district_df['District'].dropna().unique().tolist()))
...
if district_choice:
    ...
    region_options = [""] + district_regions  # blank means all in district
else:
...
if go:
    with st.spinner("Loading standings..."):
        selected_regions_list = None
        if district_choice:
            if region_choice:
                selected_regions_list = [region_choice]
            else:  # blank region => all regions in district
                selected_regions_list = district_regions
        raw_data, has_results = gather_data(group_choice, region_choice, selected_regions_list)
